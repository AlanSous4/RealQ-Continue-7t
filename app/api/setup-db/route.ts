...
export async function POST() {
  try {
    ...
    const { error: characteristicsError } = await supabaseClient.rpc("exec_sql", {
      sql_string: `
        ...
        CREATE POLICY "Usuários autenticados podem ler todas as características de inspeção" 
        ON public.inspection_characteristics FOR SELECT 
        TO authenticated 
        USING (true);
        
        CREATE POLICY "Usuários autenticados podem inserir características de inspeção" 
        ON public.inspection_characteristics FOR INSERT 
        TO authenticated 
        WITH CHECK (true);
        
        CREATE POLICY "Usuários autenticados podem atualizar características de inspeção" 
    ...
    // Adicionar campo de categoria às não conformidades (se não existir)
    const { error: nonConformityCategoryError } = await supabaseClient.rpc("exec_sql", {
      sql_string: `
        DO $$
        BEGIN
          IF NOT EXISTS (
    ...
    // Adicionar campo de não conformidade aos planos de ação (se não existir)
    const { error: actionPlanNonConformityError } = await supabaseClient.rpc("exec_sql", {
      sql_string: `
        DO $$
        BEGIN
          IF NOT EXISTS (
    ...
    // Inserir testes padrão (se não existirem)
    const { error: testsError } = await supabaseClient.rpc("exec_sql", {
      sql_string: `
