...
import { getAllTools, deleteTool, type Tool } from "@/lib/services/tool-service"

interface ToolListProps {
  searchTerm: string
}

export function ToolList({ searchTerm }: ToolListProps) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      // Atualizar a lista de ferramentas após a exclusão
      ...
      toast({
        ...
        description:
          error instanceof Error ? error.message : "Ocorreu um erro ao excluir a ferramenta. Tente novamente.",
        variant: "destructive",
      })
    } finally {
      setIsDeleting(false)
          ...
          {filteredTools.length > 0 ? (
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem asChild>
                        <Link href={`/dashboard/ferramentas/${tool.id}`}>
                          <Eye className="mr-2 h-4 w-4" />
                          Ver detalhes
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem asChild>
                        <Link href={`/dashboard/ferramentas/${tool.id}/editar`}>
                      ...
                      <DropdownMenuItem
                        ...
                        onSelect={() => setDeleteToolId(tool.id)}
                      >
            ...
            <TableRow>
              <TableCell colSpan={4} className="h-24 text-center">
                {searchTerm ? "Nenhuma ferramenta encontrada com o termo de busca." : "Nenhuma ferramenta cadastrada."}
              </TableCell>
            </TableRow>
          )}
        </TableBody>
      ...
      <AlertDialog open={!!deleteToolId} onOpenChange={(open) => !open && setDeleteToolId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. Isso excluirá permanentemente a ferramenta.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
